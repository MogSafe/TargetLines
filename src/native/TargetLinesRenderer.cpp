...
constexpr unsigned long long kNativeLogMaxSize = 5ull * 1024ull * 1024ull;
...
void append_log_file(char const* path, char const* message) {
    if (!path || path[0] == '\0' || !message) {
        return;
    }
    ...
    if (GetFileAttributesExA(path, GetFileExInfoStandard, &attributes)) {
        ...
        if (size >= kNativeLogMaxSize) {
            char backup[MAX_PATH] {};
            std::snprintf(backup, sizeof(backup), "%s.1", path);
            DeleteFileA(backup);
            // Rotation is best-effort because multibox processes can race here.
            // A failed move leaves the active log intact and never affects rendering.
            MoveFileExA(path, backup, MOVEFILE_REPLACE_EXISTING);
        }
    }
...
class TargetLinesRenderer final {
public:
    void Load(HMODULE module) {
        g_module = module;
        module_log_path(log_path_, sizeof(log_path_));
        InitializeCriticalSection(&direct_state_lock_);
        direct_state_lock_initialized_ = true;
        initialize_geometry_tables();
        append_log("native renderer loaded");
    }
...
#endif
        append_log("native renderer unloaded");
        if (direct_state_lock_initialized_) {
            DeleteCriticalSection(&direct_state_lock_);
            direct_state_lock_initialized_ = false;
        }
    }
    ...
    void Command(const char* command) {
        ...
        if (std::strcmp(command, "count") == 0) {
            char message[512] {};
            std::snprintf(message, sizeof(message), "lines=%d", cached_line_count_);
            append_log(message);
            return;
        }
        ...
        if (std::strcmp(command, "path") == 0) {
            char message[1024] {};
            std::snprintf(message, sizeof(message), "transport=direct state_bound=%s state_id=%s log=%s",
                state_bound_ ? "true" : "false",
                state_bound_ ? state_identifier_ : "none",
                log_path_);
            append_log(message);
        ...
        if (std::strcmp(command, "renderstats") == 0) {
            char message[512] {};
            std::snprintf(message, sizeof(message), "render_calls=%lu cached_line_count=%d", render_calls_, cached_line_count_);
            append_log(message);
            return;
        }
        ...
        if (std::strcmp(command, "matrixnext") == 0) {
            ...
            append_log("matrix probe queued for next scene frame");
            return;
        }
    ...
    void Render(IDirect3DDevice8* device) {
        ...
        if (matrix_probe_pending_) {
            ...
            probe_matrices("scene_frame");
        }
    }

    bool SetIdentity(char const* identifier) {
        if (!identifier || identifier[0] == '\0') {
            UnbindState();
            return true;
        }
        if (!valid_state_identifier(identifier)) {
            append_log("native identity rejected invalid identifier");
            return false;
        }
    ...
    bool ReplaceState(char const* state, std::size_t state_size) {
        if (!state || state_size == 0 || state_size >= direct_state_capacity_
            || !direct_state_lock_initialized_) {
            return false;
        }

        EnterCriticalSection(&direct_state_lock_);
        LONG const active = InterlockedCompareExchange(&direct_state_active_, 0, 0);
        int const inactive = active == 0 ? 1 : 0;
        std::memcpy(direct_state_buffers_[inactive], state, state_size);
        direct_state_buffers_[inactive][state_size] = '\0';
        direct_state_sizes_[inactive] = state_size;
        MemoryBarrier();
        InterlockedExchange(&direct_state_active_, inactive);
        InterlockedIncrement(&direct_state_generation_);
        LeaveCriticalSection(&direct_state_lock_);
        return true;
    }
    ...
    void Status(char* output, std::size_t output_size) const {
        if (!output || output_size == 0) {
            return;
        }
        std::snprintf(output, output_size,
            "transport=direct, state=%s, state_id=%s, cached_lines=%d, cached_rings=%d, device=%s, render_calls=%lu",
            state_bound_ ? "bound" : "unbound",
            state_bound_ ? state_identifier_ : "none",
            cached_line_count_, cached_ring_count_, d3d_device_ ? "ready" : "pending",
            render_calls_);
    }
...
private:
    void probe_device(const char* reason) {
        char message[256] {};
        std::snprintf(message, sizeof(message), "device probe reason=%s device=%p",
            reason ? reason : "unknown", static_cast<void*>(d3d_device_));
        append_log(message);
    }
    ...
    void probe_ffxi_interface() {
        append_log("ffxiprobe unavailable in native-addon renderer");
    }
    ...
    void draw_lines() {
        ...
        if (boneprobe_requested_ && line_count > 0) {
            ...
            if (now_ms - last_boneprobe_ms_ > 1000) {
                ...
                append_log("boneprobe requested by direct state");
    ...
    static DWORD WINAPI diagnostic_bone_probe_thread_entry(void* context) {
        TargetLinesRenderer* self = static_cast<TargetLinesRenderer*>(context);
    ...
    int read_state(LineState* lines, int max_lines, RingState* rings, int max_rings) {
        return read_direct_state(lines, max_lines, rings, max_rings);
    }

    int read_direct_state(LineState* lines, int max_lines, RingState* rings, int max_rings) {
        LONG generation = InterlockedCompareExchange(&direct_state_generation_, 0, 0);
        if (generation == direct_state_consumed_generation_ && state_cache_valid_) {
            return copy_cached_state(lines, max_lines, rings, max_rings);
        }

        char buffer[direct_state_capacity_] {};
        std::size_t state_size = 0;
        EnterCriticalSection(&direct_state_lock_);
        generation = InterlockedCompareExchange(&direct_state_generation_, 0, 0);
        LONG const active = InterlockedCompareExchange(&direct_state_active_, 0, 0);
        if (active >= 0 && active < 2) {
            state_size = direct_state_sizes_[active];
            if (state_size < direct_state_capacity_) {
                std::memcpy(buffer, direct_state_buffers_[active], state_size + 1);
            } else {
                state_size = 0;
            }
        }
        ...
        if (state_size == 0 || !parse_state_buffer(buffer, state_size, max_lines)) {
            return state_cache_valid_ ? copy_cached_state(lines, max_lines, rings, max_rings) : 0;
        }

        direct_state_consumed_generation_ = generation;
        return copy_cached_state(lines, max_lines, rings, max_rings);
    }

    bool parse_state_buffer(char* buffer, std::size_t read, int max_lines) {
        std::size_t content_end = read;
        while (content_end > 0 && (buffer[content_end - 1] == ' '
            || buffer[content_end - 1] == '\t' || buffer[content_end - 1] == '\r'
    ...
    bool begin_draw_state() {
        ...
        const HRESULT stream_result =
            d3d_device_->GetStreamSource(0, &saved_stream_, &saved_stream_stride_);

        const bool captured = SUCCEEDED(shader_result) && SUCCEEDED(alpha_result)
            && SUCCEEDED(src_result) && SUCCEEDED(dest_result) && SUCCEEDED(z_result)
            && SUCCEEDED(lighting_result) && SUCCEEDED(cull_result)
            && SUCCEEDED(texture_result) && SUCCEEDED(stream_result);
        if (!captured) {
            if (saved_texture_) {
        ...
        const HRESULT shader_set =
            d3d_device_->SetVertexShader(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);

        const bool configured = SUCCEEDED(texture_set) && SUCCEEDED(alpha_set)
            && SUCCEEDED(src_set) && SUCCEEDED(dest_set) && SUCCEEDED(z_set)
            && SUCCEEDED(lighting_set) && SUCCEEDED(cull_set) && SUCCEEDED(shader_set);
        if (!configured) {
            end_draw_state();
            return false;
        }
    ...
    void append_log(const char* message) {
        append_log_file(log_path_, message);
    }
...
#endif
    static constexpr std::size_t direct_state_capacity_ = 131072;
    CRITICAL_SECTION direct_state_lock_ {};
    bool direct_state_lock_initialized_ = false;
    char direct_state_buffers_[2][direct_state_capacity_] {};
    std::size_t direct_state_sizes_[2] {};
    volatile LONG direct_state_active_ = 0;
    volatile LONG direct_state_generation_ = 0;
    LONG direct_state_consumed_generation_ = -1;
...
TargetLinesRenderer* targetlines_renderer_create(HMODULE module) {
    auto* renderer = new TargetLinesRenderer();
    renderer->Load(module);
    return renderer;
}
...
void targetlines_renderer_status(
    ...
    if (!renderer) {
        std::snprintf(output, output_size, "renderer unavailable");
        return;
    }
    renderer->Status(output, output_size);
}
